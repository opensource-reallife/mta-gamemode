...
function HighStakeRoulette:placeToken(field)
    ...
    if self:calcBet() + ROULETTE_TOKENS[color] > (getElementData(root, "HighStakeMaxBet") or 500000) then
        ErrorBox:new(_("Der maximale Einsatz beträgt $%s!", convertNumber(getElementData(root, "HighStakeMaxBet") or "500000")))
        return
    end
...
function HighStakeRoulette:updateBetGui()
    HighStakeRouletteGUI:getSingleton().m_BetLabel:setText(("Gesamter Einsatz:\n%s"):format(toMoneyString(self:calcBet())))
end
...
function HighStakeRoulette:createTokens()
    ...
    for color, amount in spairs(ROULETTE_TOKENS, function(t,a,b) return t[b] > t[a] end) do
        ...
        self.m_Token[color].onHover = function() self.m_InfoLabel:setText(("%ser Jetton - Wert pro Stück: %s"):format(HighStakeRoulette.ColorNames[color], toMoneyString(amount))) end
