...
function GroupCreationGUI:constructor()
	...
	GUILabel:new(self.m_Width*0.01, self.m_Height*0.16, self.m_Width*0.98, self.m_Height*0.04, _("Hier kannst du eine Gruppe gründen.\nEine Gründung kostet %s!", toMoneyString(GROUP_CREATE_COSTS)), self.m_Window):setFont(VRPFont(self.m_Height*0.11)):setMultiline(true)
	GUILabel:new(self.m_Width*0.01, self.m_Height*0.42, self.m_Width*0.65, self.m_Height*0.10, _"Name der Gruppe:", self.m_Window):setFont(VRPFont(self.m_Height*0.12))
	...
	self.m_Type:addItem(_"Gang")
	self.m_Type:addItem(_"Firma")
