...
function MapEditorMapSettingsGUI:constructor(id, mapInfos)
	...
	self.m_CategoryLabel = GUIGridLabel:new(1, 5, 4, 1, "Kategorie:", self.m_Window)
	...
	self.m_CategoryChanger = GUIGridChanger:new(5, 5, 7, 1, self.m_Window)
    self.m_SelectedCategory = 0

	self.m_SaveButton = GUIGridButton:new(4, 6, 4, 1, "Speichern", self.m_Window)
    self.m_DiscardButton = GUIGridButton:new(8, 6, 4, 1, "Abbrechen", self.m_Window)
    ...
    self.m_SaveButton.onLeftClick = function()
        ...
        local settings = {id, name, activate, saveObjects, deactivatable, self.m_SelectedCategory}
...
function MapEditorMapSettingsGUI:fillInfos(mapInfos)
    ...
    self.m_CategoryChanger.m_Items[1] = "Keine"
    for i, v in pairs(MapEditor:getSingleton().m_Categories) do
        self.m_CategoryChanger:addItem(v)
    end

    self.m_CategoryChanger.onChange = function(text, index)
        self.m_SelectedCategory = index - 1
    end
    self.m_CategoryChanger:setIndex(mapInfos[6] + 1)
