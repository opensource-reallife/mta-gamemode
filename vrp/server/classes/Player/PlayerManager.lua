...
function PlayerManager:checkPayday()
	for k, v in pairs(self:getReadyPlayers()) do
...
function PlayerManager:getReadyPlayers()
	for index, playeritem in pairs(self.m_ReadyPlayers) do
		if not isElement(playeritem) then
			self.m_ReadyPlayers[index] = nil
		end
	end
...
function PlayerManager:sendShortMessage(text, ...)
	for k, player in pairs(self:getReadyPlayers()) do
				...
				if killer:isFactionDuty() then
					...
					if wantedLevel > 0 then
						if killer:getPublicSync("gangwarParticipant") and client:getPublicSync("gangwarParticipant") then
							...
							self:sendShortMessage(_("%s wurde soeben von %s für %d Minuten eingesperrt! Strafe: %d$", client, client:getName(), killer:getName(), jailTime, factionBonus), "Staat")
...
function PlayerManager:Event_gunBoxAddWeapon(weaponId, muni)
	...
	if weaponId == 38 then client:sendError(_("Du kannst diese Waffe nicht einlagern!", client)) return end
