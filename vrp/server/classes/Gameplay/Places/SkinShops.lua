...
function SkinShops:constructor()
	...
	blip:setDisplayText("Kleidungsgeschäft (Secondhand)", BLIP_CATEGORY.Shop)
	...
	blip:setDisplayText("Kleidungsgeschäft (Luxus)", BLIP_CATEGORY.Shop)
	blip:setOptionalColor({217, 240, 224})
...
function SkinShops:Event_skinBuy(skinId)
	...
	if source:getMoney() >= price then
		if self:giveClothItem(source, skinId) then
...
function SkinShops:giveClothItem( player, skinID )
	...
	-- 	end
	-- end
	return inv:giveItem("Kleidung",1,skinID)
end
