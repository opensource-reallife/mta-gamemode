...
function InactivityManager:clearHouses()
	local rows = sql:queryFetch("SELECT hs.Id as HouseID, ac.Id as UserID FROM ??_houses hs INNER JOIN ??_account ac ON ac.Id = hs.owner WHERE ac.LastLogin < DATE_SUB(DATE(NOW()), INTERVAL 6 MONTH);", sql:getPrefix(), sql:getPrefix())
...
function InactivityManager:isGroupInactive(id)
	local result, numrows = sql:queryFetch("SELECT ac.LastLogin FROM ??_character ch INNER JOIN ??_account ac ON ac.Id = ch.Id AND ch.GroupId = ? WHERE ac.LastLogin > DATE_SUB(DATE(NOW()), INTERVAL 6 MONTH)", sql:getPrefix(), sql:getPrefix(), id)
	if numrows == 0 then
		return true
	else
		return false
