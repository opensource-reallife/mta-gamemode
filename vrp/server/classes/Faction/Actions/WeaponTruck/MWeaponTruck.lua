...
function MWeaponTruck:constructor()
	...
	self.m_Evilwt = Blip:new("Logistician.png", -2103.16, -2276.99, {factionType = "Evil" and "Rescue"}, 400)
	self.m_Evilwt:setDisplayText("Waffentruck")
	self.m_Evilwt:setOptionalColor({27, 125, 47})

	self.m_Statewt = Blip:new("Logistician.png", 2736.51, -2457.10, {factionType = "State" and "Rescue"}, 400)
	self.m_Statewt:setDisplayText("Staatswaffentruck")
	self.m_Statewt:setOptionalColor({27, 125, 47})
end
	...
	if ActionsCheck:getSingleton():isActionAllowed(client) then
		...
		if faction:isEvilFaction() then
			if client:isFactionDuty() then
				if FactionState:getSingleton():countPlayers(true, false) < WEAPONTRUCK_MIN_MEMBERS then
					client:sendError(_("Es müssen mindestens %d Staatsfraktionisten aktiv sein!", client, WEAPONTRUCK_MIN_MEMBERS))
					return
				end
		...
		elseif faction:isStateFaction() then
			if client:isFactionDuty() then
				if FactionEvil:getSingleton():countPlayers(true, false) < WEAPONTRUCK_MIN_MEMBERS then
					client:sendError(_("Es müssen mindestens %d Spieler böser Fraktionen aktiv sein!", client, WEAPONTRUCK_MIN_MEMBERS))
					return
				end
