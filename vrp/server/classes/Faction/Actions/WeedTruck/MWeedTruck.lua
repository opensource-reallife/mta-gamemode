...
function MWeedTruck:constructor()
	...
	self.m_BlipWeedTruck:setDisplayText("Drogentruck", BLIP_CATEGORY.Actions)
			...
			if faction:isEvilFaction() and hitElement:isFactionDuty() then
				if PermissionsManager:getSingleton():isPlayerAllowedToStart(hitElement, "faction", "WeedTruck") then
					if ActionsCheck:getSingleton():isActionAllowed(hitElement) then
						if FactionState:getSingleton():countPlayers(true, false) < WEEDTRUCK_MIN_MEMBERS then
							hitElement:sendError(_("Es müssen mindestens %d Staatsfraktionisten aktiv sein!", hitElement, WEEDTRUCK_MIN_MEMBERS))
							return false
						end
